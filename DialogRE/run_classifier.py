...
def main():
    ...
    if args.do_eval:
        ...
        for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
            ...
            input_mask = torch.sum(input_mask, dim=2).squeeze(-1)

            input_mask = input_mask.detach().cpu().numpy()

            for i in range(len(logits)):
                length_all += [input_mask[i]]
            ...
            for i in range(len(logits_all)):
                for j in range(len(logits_all[i])):
                    ...
                    if j == len(logits_all[i]) - 1:
                        ...
                        f.write(" ")
        # print(length_all)
        output_length_file = os.path.join(args.output_dir, "length_dev.txt")
        with open(output_length_file, "w") as f:
            for i in range(len(length_all)):
                f.write(str(length_all[i]))
                f.write("\n")
