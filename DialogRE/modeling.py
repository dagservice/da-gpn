...
from multi_view_gcn import MultiRelationalGCN
...
class BertForSequenceClassification(nn.Module):
    def __init__(self, config, num_labels, args):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.gcn = MultiRelationalGCN(args, config)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None, n_class=1):
        ...
        h_out, pool_mask, layer_list, src_mask_input = self.gcn(adj, word_embedding, input_ids.view(-1,seq_length))

        outputs = torch.cat([pooled_output, h_out], dim=1)

        # h_out, pool_mask, layer_list, subj_gcn_entity, obj_gcn_entity = self.gcn(adj, word_embedding, input_ids, s_mask,
        #                                                                          o_mask)
        #
        ...
        if labels is not None:
            ...
            # B, T, C = h.size()
            # h1 = h.unsqueeze(1).repeat(1, T, 1, 1)
            # h2 = h.unsqueeze(2).repeat(1, 1, T, 1)
            # dis = torch.pow(h1 - h2, 2).sum(3).sum(-1).sum(-1).mean()
            # dis_loss = 1.0 / dis
            # loss = loss_fct(logits.float(), labels) + self.args.lamada * dis_loss
