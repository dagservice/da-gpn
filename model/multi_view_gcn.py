...
class MultiRelationalGCN(nn.Module):
    ...
    def forward(self, adj, inputs, input_ids, subj_mask, obj_mask):
        ...
        _, attn_adj_list = self.GraphGenerate(gcn_inputs, adj, src_mask)
        ...
        subj_entity = self.entity_max(aggregate_out, subj_mask)
        obj_entity = self.entity_max(aggregate_out, obj_mask)

        return out, mask_out, layer_list, subj_entity, obj_entity
...
class GraphGenerate(nn.Module):
    def __init__(self, args, config):
        ...
        self.subj_transform = nn.Linear(args.graph_hidden_size, args.graph_hidden_size)
        self.obj_transform = nn.Linear(args.graph_hidden_size, args.graph_hidden_size)
    ...
    def forward(self, x, adj, mask=None):
        ...
        adj_multi = torch.softmax(torch.relu(rep_multi), dim=2)
